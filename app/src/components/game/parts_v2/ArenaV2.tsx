...
export function ArenaV2({ kiosk, arena, end }: ArenaProps) {
  // we play audio here.
  ...
  const getGameStatus = async (arenaId: string) => {
    ...
    if (
      ...
      // now we reveal.
      await reveal(
        Number(localStorage.getItem("lastMove") as string),
        arena,
        kiosk
      );
  ...
  return (
    <div
      className="bg-contain py-12 min-h-[600px] before:bg-black text-white before:bg-opacity-40"
    ...
    >
      ...
      {!result && (
        <div>
          ...
          {!isExpectingMove && (
            <div className="text-center py-12 block mt-3">
