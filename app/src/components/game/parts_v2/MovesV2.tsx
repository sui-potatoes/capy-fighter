...
export function MovesV2({ playerStats, makeMove }: MoveV2Props) {
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-5 px-6">
        {getMoves().map((move, index) => {
          return (
            <button
              key={index}
              className="bg-transparent border-black bg-black bg-opacity-50 py-3 md:py-6 rounded-lg"
              onKeyUp={(e) => {
                console.log(e);
              }}
              onClick={() => {
            ...
            >
              ...
              <p>
                {move.move.name} ({move.move.category}) -{" "}
                {eventCodes[index].replace("Key", "")}
              </p>
            </button>
          );
        })}
