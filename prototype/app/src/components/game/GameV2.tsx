...
export function GameV2() {
  ...
  const { player, isInGame, loading, getData, reset, setLoading } =
    useUserPlayer({ extension });
  ...
  if (balance === 0)
    return <p>Please use the faucet before you can play the game!</p>;
  ...
  if (player && !gameStarted)
    return <>
      <StartGameScreen disabled={!kiosk} start={start} />
      {/* <SinglePlayerStats player={player} isCurrent={true} /> */}
    </>
